...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups # pg0 batchnorm parameters 
    ...
    compute_loss = ComputeLoss(model,pruning=opt.slimming,sr=opt.sr)  # init loss class
    logger.info(f'Image sizes {imgsz} train, {imgsz_test} test\n'
                f'Using {dataloader.num_workers} dataloader workers\n'
                f'Slimming: {opt.slimming}\n'
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.slimming:
            logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'gamma', 'total', 'labels', 'img_size'))
        else:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov5s-person.yaml', help='model.yaml path')
    ...
    parser.add_argument('--slimming', action='store_true', help='train model with l1 sparsity')
